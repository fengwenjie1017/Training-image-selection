...
int ReadPar(CString ParaFile)
{
	...
	else
	{
		...
		if(CheckPf) cout<<"Number of level of similarity analysis:   "<<NLevel<<endl;
		//不同尺度的搜索窗口大小（动态分配与读取）
...
void WriteVersion()
{
	...
	cout<<"                   by Wenjie Feng  V1.0\n";
	cout<<"               Data: 2016-08-09-11:34:00\n";
	cout<<"***********************************************************\n"<<endl;
}
							...
							if(GridValue[RR[order].rpz+i-sz/2][RR[order].rpy+j-sy/2][RR[order].rpx+k-sx/2].discprop>-1)
								{
									...
									//针对不同类型的变量给定GP内部各点的属性
									GP[m].facies=GridValue[RR[order].rpz+i-sz/2][RR[order].rpy+j-sy/2][RR[order].rpx+k-sx/2].discprop;
									MM = GridValue[RR[order].rpz + i - sz / 2][RR[order].rpy + j - sy / 2][RR[order].rpx + k - sx / 2].discprop;
									//计算GP内部各点的相对坐标
									GP[m].px=k-sx/2;GP[m].py=j-sy/2;GP[m].pz=i-sz/2;
									//计数器
									m++;
									//调试输出
									//cout<<"PatternConstruction: 数量"<<m-1<<"坐标"<<GP[m-1].px<<"  "<<GP[m-1].py<<"   "<<GP[m-1].pz<<endl;
									//cout <<"GP order="<<order<<"    m-order="<<m<< "  facies=" << GP[m-1].facies <<"   mm="<<MM<< "   gridvalue=" << GridValue[RR[order].rpz + i - sz / 2][RR[order].rpy + j - sy / 2][RR[order].rpx + k - sx / 2].discprop << endl;
									//Sleep(50);
								}
						if(DistanceType==2)
							if(GridValue[RR[order].rpz+i-sz/2][RR[order].rpy+j-sy/2][RR[order].rpx+k-sx/2].contprop>-1)
...
//	if(TIType==0)   fout << "Last TI\n";   //输出最后一幅训练图像
	for(j=0;j<NTI;j++)
		for(int i=0;i<NLevel;i++)
			fout<<"MD_TI"<<TrainingImageFl[j]<<"SWS_"<<searchwindowscale[i][0]<<"*"<<searchwindowscale[i][1]<<"*"<<searchwindowscale[i][2]<<"_timecost="<<timecost[j][i]<<"ms"<<"\n";
